...
io.on('connection', socket => {
    ...
    socket.join(user.room) // joins to specific room
		...
		socket.emit('message', generateMessage(user.username, `Welcome to room: ${options.room}`))
    ...
    if(usersInRoom.length === 1)return callback('No users in this room!')

    const locationURL = `https://www.google.com/maps?q=${locationData.latitude},${locationData.longitude}`
    
    io.to(user.room).emit('location', generateLocationMessage(user.username, locationURL))
